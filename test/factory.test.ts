...
describe('Test Suite', function () {
    ...
    function proposal() {
        ...
        PROPOSAL.addedToks.tokenA2TaxOnBuy = 500;
        PROPOSAL.addedToks.tokenB2TaxOnBuy = 0;
        PROPOSAL.addedToks.refundOnSell = 0;
        PROPOSAL.addedToks.refundOnBuy = 0;
        PROPOSAL.addedToks.tokenB1SellToLI = 500;
        PROPOSAL.addedToks.tokenB1BuyToLI = 0;
        PROPOSAL.addedToks.tokenB2SellToLI = 0;
        PROPOSAL.addedToks.tokenB2BuyToLI = 0;
    ...
    async function deployFixture() {
        const [owner, addr1, addr2, tokenomicsFeeReceiver] =
        ...
        const darwin = (await erc20Factory.deploy(
            'Darwin',
            'DARWIN',
            owner.address
        )) as TestERC20;
        ...
        const pairFactory = await ethers.getContractFactory('DarwinSwapPair', {
            libraries: { Tokenomics2Library: library.address },
        });
    ...
    describe('Tdajkdsdadsa', function () {
        it('bla bla', async function () {
            ...
            const pairContract = pairFactory.attach(
                darwinPair
            ) as DarwinSwapPair;
            const injector = await pairContract.liquidityInjector();
            console.log(`Injector: ${injector}`);

            const injectorBalance = await pairContract.provider.getBalance(
                injector
            );
            console.log(
                `Injector Balance: ${ethers.utils.formatEther(
                    injectorBalance
                )} ETH`
            );

            const tokenInfo = await lister.tokenInfo(token.address);
            ...
            console.log(
                `Token Balance: ${ethers.utils.formatEther(tokenBalance)} TKN`
            );

            // attempt to do a sell
            ...
            const injectorBalanceAfter = await pairContract.provider.getBalance(
                injector
            );

            console.log(injectorBalanceAfter);

            console.log(
                `Injector Balance: ${ethers.utils.formatEther(
                    injectorBalanceAfter
                )} ETH`
            );
        });
    });
    ...
    describe('Factory', function () {
        it('The function `getPair(address,address)` returns `address(0)` if the pair does not exist', async function () {
            ...
            expect(
                await factory.getPair(weth.address, token.address)
            ).to.be.equal(ZERO);
        });

        it('When a pair is created, its `token0()` and `token1()` equal to the addresses used to create it', async function () {
            const { lister, weth, token, pairFactory } = await loadFixture(
                deployFixture
            );
            const pairAddress = await lister.callStatic.createPair(
                weth.address,
                token.address
            );
            expect(
                await lister.createPair(weth.address, token.address)
            ).to.not.be.equal(ZERO);
            const pair = pairFactory.attach(pairAddress) as DarwinSwapPair;
            const [tokenA, tokenB] =
        ...
        it('The function `allPairsLength()` correctly keeps track of all the created pairs', async function () {
            const { factory, lister, weth, token } = await loadFixture(
                deployFixture
            );
    ...
    describe('Router', function () {
        it('AddLiquidityETH works', async function () {
            const { erc20Factory, router, owner } = await loadFixture(
                deployFixture
            );
            const token = (await erc20Factory.deploy(
                'Test Token 2',
        ...
        it('Tokenomics Potential Exploit', async function () {
            const { router, owner, weth, darwin, darwinPair, pairFactory } =
                await loadFixture(pairFixture);
            const swaps = 3;
            const balanceBefore = await darwin.balanceOf(owner.address);
            const ethBalanceBefore = await owner.getBalance();
            console.log(
                '          1️⃣  ETH Balance (before): ' +
                    ethers.utils.formatEther(ethBalanceBefore)
            );
            console.log(
                '          2️⃣  Darwin Balance (before): ' +
                    ethers.utils.formatEther(balanceBefore)
            );
            console.log(
                `          3️⃣  Pair Balance (before): ${ethers.utils.formatEther(
                    await darwin.balanceOf(darwinPair)
                )} DARWIN - ${ethers.utils.formatEther(
                    await weth.balanceOf(darwinPair)
                )} ETH`
            );
            console.log('');
            await router.swapExactETHForTokensSupportingFeeOnTransferTokens(
                eth('0'),
                [weth.address, darwin.address],
                owner.address,
                (await time.latest()) + 1000,
                { value: eth('500') }
            );
            ...
            const adg = await pairContract.liquidityInjector();
            const adgFactory = await ethers.getContractFactory(
                'LiquidityInjector'
            );
            const adgContract = adgFactory.attach(adg);
            console.log(
                '          1️⃣  ETH Balance (before inject): ' +
                    ethers.utils.formatEther(await owner.getBalance())
            );
            console.log(
                '          2️⃣  Darwin Balance (before inject): ' +
                    ethers.utils.formatEther(
                        await darwin.balanceOf(owner.address)
                    )
            );
            console.log(
                `          3️⃣  Pair Balance (before inject): ${ethers.utils.formatEther(
                    await darwin.balanceOf(darwinPair)
                )} DARWIN - ${ethers.utils.formatEther(
                    await weth.balanceOf(darwinPair)
                )} ETH`
            ...
            console.log(
                '          1️⃣  ETH Balance of ADG (before inject): ' +
                    ethers.utils.formatEther(await weth.balanceOf(adg))
            );
            console.log(
                '          2️⃣  Darwin Balance of ADG (before inject): ' +
                    ethers.utils.formatEther(await darwin.balanceOf(adg))
            );
            console.log('');
            ...
            console.log(
                '          1️⃣  ETH Balance (after inject): ' +
                    ethers.utils.formatEther(await owner.getBalance())
            );
            console.log(
                '          2️⃣  Darwin Balance (after inject): ' +
                    ethers.utils.formatEther(
                        await darwin.balanceOf(owner.address)
                    )
            );
            console.log(
                `          3️⃣  Pair Balance (after inject): ${ethers.utils.formatEther(
                    await darwin.balanceOf(darwinPair)
                )} DARWIN - ${ethers.utils.formatEther(
                    await weth.balanceOf(darwinPair)
                )} ETH`
            ...
            console.log(
                '          1️⃣  ETH Balance of ADG (after inject): ' +
                    ethers.utils.formatEther(await weth.balanceOf(adg))
            );
            console.log(
                '          2️⃣  Darwin Balance of ADG (after inject): ' +
                    ethers.utils.formatEther(await darwin.balanceOf(adg))
            );
            console.log('');
            ...
            console.log(
                '          1️⃣  ETH Balance (after): ' +
                    ethers.utils.formatEther(ethBalanceAfter)
            );
            console.log(
                '          2️⃣  Darwin Balance (after): ' +
                    ethers.utils.formatEther(balanceAfter)
            );
            console.log(
                `          3️⃣  Pair Balance (after): ${ethers.utils.formatEther(
                    await darwin.balanceOf(darwinPair)
                )} DARWIN - ${ethers.utils.formatEther(
                    await weth.balanceOf(darwinPair)
                )} ETH`
    ...
    describe('Lister', function () {
        ...
        describe('Security', function () {
            it('Anyone can create a pair', async function () {
                const { lister, addr1, weth, token } = await loadFixture(
                    deployFixture
                );
                expect(
                    await lister
                        .connect(addr1)
                        .createPair(weth.address, token.address)
                ).to.not.be.reverted;
            });

            it('When a pair is created, if `createPair(address,address)` is called again with the same tokens, it reverts', async function () {
                const { lister, weth, token } = await loadFixture(
                    deployFixture
                );
                await lister.createPair(weth.address, token.address);
                await expect(
                    lister.createPair(weth.address, token.address)
                ).to.be.revertedWith('DarwinSwap: PAIR_EXISTS');
            });

            it('Tokenomics proposal can only be made either by the token owner or by the dev address', async function () {
                const { lister, addr1, addr2, erc20Factory } =
                    await loadFixture(deployFixture);
                const token1 = (await erc20Factory
                    .connect(addr1)
                    .deploy(
                        'Test Token 1',
                        'TTKN1',
                ...
                await expect(
                    await lister
                        .connect(addr1)
                        .listToken(token1.address, PROPOSAL)
                ).to.not.be.reverted;
                const token2 = (await erc20Factory
                    .connect(addr1)
                    .deploy(
                        'Test Token 2',
                        'TTKN2',
                ...
                await expect(
                    lister.connect(addr2).listToken(token2.address, PROPOSAL)
                ).to.be.reverted;
                await expect(await lister.listToken(token2.address, PROPOSAL))
                    .to.not.be.reverted;
            });
        });
        ...
        describe('Token Listing', function () {
            it('Tokens can set Tokenomics 1.0 when listing a token on buys or sells, and on coins or tokens, up to 10% per buy or sell', async function () {
                const { lister, token, token1 } = await loadFixture(
                    deployFixture
                );
                let prop = proposal();
                prop.addedToks.tokenA1TaxOnSell = 1000;
                await expect(await lister.listToken(token.address, PROPOSAL)).to
                    .not.be.reverted;
                prop.addedToks.tokenA1TaxOnSell = 1001;
                await expect(
                    lister.listToken(token1.address, PROPOSAL)
                ).to.be.revertedWith(
                    'DarwinSwap: INVALID_REQUESTED_TOKENOMICS'
                );
            });
            ...
            it('Tokenomics 1.0 takes tokens from the user', async function () {
                ...
                const { owner, router, token, weth, pair, addedToks } =
                    await loadFixture(pairFixture);
                const arrivedAmount = eth(SWAPAMOUNT).sub(
                    eth(SWAPAMOUNT).mul(addedToks.tokenA1TaxOnSell).div(10000)
                );
                ...
                await expect(
                    ...
                    .to.changeTokenBalances(
                        token,
                        [owner.address, pair.address],
                        [eth('-' + SWAPAMOUNT), arrivedAmount]
                    )
                    .changeTokenBalances(
                        weth,
                        [owner.address, pair.address],
            ...
            it('Tokens can set Tokenomics 2.0 when listing a token on buys or sells, and on coins or tokens, up to 5% per buy or sell', async function () {
                const { lister, token, token1 } = await loadFixture(
                    deployFixture
                );
                let prop = proposal();
                prop.addedToks.tokenA2TaxOnSell = 500;
                await expect(await lister.listToken(token.address, prop)).to.not
                    .be.reverted;
                prop.addedToks.tokenA2TaxOnSell = 501;
                await expect(
                    lister.listToken(token1.address, prop)
                ).to.be.revertedWith(
                    'DarwinSwap: INVALID_REQUESTED_TOKENOMICS'
                );
            });
            ...
            it('Tokenomics 2.0 takes tokens from the liquidity pool', async function () {
                const SWAPAMOUNT = '1';
                fixtureProp = proposal();
                fixtureProp.addedToks.tokenA2TaxOnSell = 500;
                const { owner, router, token, weth, pair, addedToks } =
                    await loadFixture(pairFixture);
                const arrivedAmount = eth(SWAPAMOUNT).sub(
                    eth(SWAPAMOUNT).mul(addedToks.tokenA2TaxOnSell).div(10000)
                );
                const reserves = await pair.getReserves();
                const amountOut = await router.getAmountOut(
                    eth(SWAPAMOUNT),
            ...
            it('When they set Tokenomics on a contract they must say what the purpose of taxing is', async function () {
                ...
                prop.purpose = '';
                await expect(
                    lister.listToken(token.address, prop)
                ).to.be.revertedWith('DarwinSwap: EMPTY_PURPOSE');
            });
            ...
            it("Tokenomics only work when the 'other' token is 'official'", async function () {
                ...
                fixtureProp = proposal();
                fixtureProp.addedToks.tokenA1TaxOnSell = 1000;
                const { token, weth, pair, router, owner } = await loadFixture(
                    unofficialPairFixture
                );
                await expect(
                    await router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        eth(SWAPAMOUNT),
                    ...
                    token,
                    [owner.address, pair.address],
                    [eth('-' + SWAPAMOUNT), eth(SWAPAMOUNT)]
                );
            });

            it('An already listed token can be relisted', async function () {
                const { weth, lister } = await loadFixture(pairFixture);
                fixtureProp = proposal();
                await lister.listOfficialToken(weth.address);
                await expect(await lister.listToken(weth.address, fixtureProp))
                    .to.not.be.reverted;
            });
        });
    });
    ...
    describe('Bundles', function () {
        it('Enter bundle works', async function () {
            const { bundles, lister, router, weth, token, owner } =
                await loadFixture(deployFixture);
            const t1 = await token.approve(
                router.address,
                ethers.utils.parseEther('1000')
            );
            await t1.wait();
            const t2 = await router.addLiquidityETH(
                token.address,
                ethers.utils.parseEther('1000'),
                0,
                0,
            ...
            await t3.wait();
            const t4 = await token.transfer(
                bundles.address,
                ethers.utils.parseEther('1000')
            );
            await t4.wait();
            expect(
                await bundles.enterBundle(
                    token.address,
        ...
        it('Enter bundle works with farm in masterchef', async function () {
            ...
            await t4.wait();
            const t5 = await masterChef.addPool(
                100,
                await factory.getPair(token.address, weth.address),
                150,
                150,
                0,
                false
    ...
    describe('MasterChef', function () {
        it('Deposit works', async function () {
            ...
            const t3 = await masterChef.addPool(
                100,
                pair.address,
                250,
                150,
            ...
            await t3.wait();
            const t4 = await pair.approve(
                masterChef.address,
                await pair.balanceOf(owner.address)
            );
            await t4.wait();
            expect(
                await masterChef.depositByLPToken(
                    pair.address,
                    await pair.balanceOf(owner.address),
                    false,
                    0

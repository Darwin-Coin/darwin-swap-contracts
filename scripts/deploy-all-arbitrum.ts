...
import {
    ...
    LootboxTicket,
    DarwinStaking,
} from '../darwin-token-contracts/typechain-types';
import {
    Darwin,
    DarwinVester,
    StakedDarwin,
} from '../darwin-token-contracts/typechain-types/contracts';
...
async function main() {
    ...
    const darwinCommunityFactory = await ethers.getContractFactory(
        'DarwinCommunity'
    );
    ...
    //! [DEPLOY] VESTER
    const vester = (await darwinVesterFactory.deploy(
        users,
        valuesAtLaunch,
        valuesDue,
        [addr.evotures]
    ...
    //! [DEPLOY] COMMUNITY
    const community = (await darwinCommunityFactory.deploy(
        addr.kieran
    )) as DarwinCommunity;
    ...
    const stakedDarwin = stakedDarwinFactory.attach(
        await darwin.stakedDarwin()
    ) as StakedDarwin;
    ...
    const burner = (await darwinBurnerFactory.deploy(
        darwin.address
    )) as DarwinBurner;
    ...
    //! [ATTACH] LOCKER
    const locker = lockerFactory.attach(
        await masterChef.locker()
    ) as TokenLocker;
    ...
    //* [INIT] COMMUNITY
    const comInit = await community.init(
        darwin.address,
        fundAddress,
        initialFundProposalStrings,

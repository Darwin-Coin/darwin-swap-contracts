...
contract DarwinSwapRouter is IDarwinSwapRouter {
    ...
    function _addLiquidity(
        ...
        if (IDarwinSwapFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IDarwinSwapLister(IDarwinSwapFactory(factory).lister()).createPair(
                tokenA,
                tokenB
            );
        }
        (uint reserveA, uint reserveB) = DarwinSwapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = DarwinSwapLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "DarwinSwapRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = DarwinSwapLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "DarwinSwapRouter: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        if (
            msg.sender !=
            address(IDarwinSwapFactory(factory).liquidityBundles())
        ) {
            IDarwinSwapFactory(factory).liquidityBundles().update(pair);
        }
    }
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            (IERC20(WETH).balanceOf(address(this)) >= amountIn)
                ? address(this)
                : msg.sender,
            DarwinSwapLibrary.pairFor(factory, path[0], path[1]),
            amountIn,
            path[path.length - 1],
            factory
        );
        for (uint i; i < path.length - 1; i++) {
            ...
            IDarwinSwapPair pair = IDarwinSwapPair(
                DarwinSwapLibrary.pairFor(factory, input, output)
            );
            uint amountInput;
            uint amountOutput;
            {
                ...
                amountInput =
                    ...
                    reserveInput;
                amountOutput = DarwinSwapLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
            ...
            address to = i < path.length - 2
                ? DarwinSwapLibrary.pairFor(factory, output, path[i + 2])
                : _to;
            {
                // scope to avoid stack too deep errors;
                address firstInPath;

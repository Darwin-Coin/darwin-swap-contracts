...
contract DarwinSwapPair is IDarwinSwapPair, DarwinSwapERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'DarwinSwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'DarwinSwap: INSUFFICIENT_LIQUIDITY');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'DarwinSwap: K');
        }
